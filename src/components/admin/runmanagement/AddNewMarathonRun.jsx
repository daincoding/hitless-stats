...
const AddNewMarathonRun = ({ player, run, onClose }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8081/admin/runs/marathon/add-run/${player}/${run.id}`, {
        method: "POST",
        headers: {
  ...
  const handleEndRun = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8081/admin/runs/marathon/end-run/${player}/${run.id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
